...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <xsl:template match="int">
    ...
    <!--    <xsl:if test="not($person=normalize-space(preceding-sibling::node()))"> -->
    <xsl:if test="string(substring-after($person, '|'))">
      <li class="name">
        <a
          href="salemSearch.htm?q=name_text:{substring-before($person, '|')}&amp;rows=20&amp;start=0&amp;name={substring-after($person, '|')}">
